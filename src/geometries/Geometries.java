...
public class Geometries extends Intersectable {
	...
	@Override
	/**
	 * Finds all intersection points between the given ray and the geometries in
	 * this collection.
	 * <p>
	 * This method supports two modes of operation, controlled by the static flag
	 * {@link renderer.SimpleRayTracer#BVH_ENABLED}:
	 * <ul>
	 * <li><b>BVH Disabled:</b> Performs a simple, exhaustive search, checking for
	 * intersections against every geometry in the collection.</li>
	 * <li><b>BVH Enabled:</b> Performs an optimized search using a two-phase
	 * Bounding Volume Hierarchy approach. It first checks if the ray intersects the
	 * bounding box of the entire collection, and only then checks against the
	 * bounding boxes of individual child geometries.</li>
	 * </ul>
	 *
	...
	public List<Intersection> calculateIntersectionsHelper(Ray ray) {
		...
		// Optimization: If the ray misses the main box, we can prune this entire branch
		// and exit early.
		...
		for (Intersectable geometry : allgeometries) {
			...
			if (geoBox == null || ray.isIntersecting(geoBox)) {
				// Optimization: Perform the expensive, detailed intersection test ONLY if the
				// ray
