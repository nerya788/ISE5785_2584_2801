...
/**
 * A factory class for creating Axis-Aligned Bounding Boxes (AABB) for geometries.
 * This class resides in the 'primitives' package to gain access to the protected
 * fields of other primitive classes like Point, which is necessary for calculations.
 */
public class BoundingBoxFactory {
    ...
    /**
     * Creates an AABB for a polygon-based geometry (like Triangle or Polygon).
     * It finds the minimum and maximum coordinates among all vertices.
    ...
    public static AABB createForCylinder(Ray axis, double radius, double height) {
        ...
        // Bounding box of the two endpoints of the axis
        AABB axisBox = new AABB(
            new Point(Math.min(p1.xyz.d1(), p2.xyz.d1()), Math.min(p1.xyz.d2(), p2.xyz.d2()), Math.min(p1.xyz.d3(), p2.xyz.d3())),
            new Point(Math.max(p1.xyz.d1(), p2.xyz.d1()), Math.max(p1.xyz.d2(), p2.xyz.d2()), Math.max(p1.xyz.d3(), p2.xyz.d3()))
        );

        // Expand the box by the radius in all directions
        Point min = new Point(axisBox.min.xyz.d1() - radius, axisBox.min.xyz.d2() - radius, axisBox.min.xyz.d3() - radius);
        Point max = new Point(axisBox.max.xyz.d1() + radius, axisBox.max.xyz.d2() + radius, axisBox.max.xyz.d3() + radius);
