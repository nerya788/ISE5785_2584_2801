...
class TubeTests {
	...
	void testFindIntersections() {
        ...
        final var exp1 = List.of(new Point (1.96332495807108, 0.26833752096446, 0),new Point (0.63667504192892, 0.93166247903554, 0));
		...
		assertNull(tube.findIntersections(new Ray(new Point(1, 0.5, 0), v001)), "Ray is parallel and inside tube, down head point");

		//TC06: Ray parallel and orthogonal to the tube axis, inside the tube, length the tube, up base (0 points)
		assertNull(tube.findIntersections(new Ray(new Point(1, 0.5, 0), v00m1)) ,"Ray is parallel and inside tube, up head point");

		// TC07: Ray parallel and orthogonal to axis, outside tube ,length the tube (0 points)
		assertNull(tube.findIntersections(new Ray(new Point(3, 0, 0), v011)), "Ray parallel and outside tube");
		
		// TC08: Ray parallel and orthogonal to axis, outside tube ,length the tube (0 points)
		assertNull(tube.findIntersections(new Ray(new Point(3, 0, 0), v001)), "Ray parallel and outside tube");
		
		//TC09: Ray orthogonal to the tube axis, inside the tube, length the tube (2 points)
		...
		//TC10: Ray orthogonal to the tube axis, inside the tube, length the tube, on the center(2 points)
		...
		final var exp4 = List.of(new Point (1.866025403784439, 0.5, 0.755983064143708), new Point (0.133974596215561, 0.5, 1.910683602522959));
        ...
        final var result6 = tube.findIntersections(new Ray(new Point (1.866025403784439, 0.5, 0.7559830641437081), vm302));
		...
		final var exp13 = List.of(new Point (0.292893218813453, -0.707106781186548, 0.292893218813453), new Point (1.707106781186548, 0.707106781186548, 1.707106781186548));
		...
		assertNull(tube.findIntersections(new Ray(new Point(1.707106781186548, 0.707106781186548, 1.707106781186548), v111)), "Ray start at the end of the tube");
		
		// TC20: Ray start began the tube(1 points)
        final var result15 = tube.findIntersections(new Ray(new Point (0.292893218813453, -0.707106781186548, 0.292893218813453), v111));
		...
		//**** Group 5: line passes inside the tube, start with length of tube on the center axis
		...
		assertNull(tube.findIntersections(new Ray(new Point(1.5, 0, 0), v001)), "Ray is orthogonal and outside vers versa center of tube");

		// TC46: Ray is orthogonal and outside vis versa center of tube(0 points)
		assertNull(tube.findIntersections(new Ray(new Point(1.5, 0, 0), v00m1)), "Ray is orthogonal and outside vers versa center of tube");
	
		// TC47: Ray is orthogonal and inside center of tube, inside the tube center(0 points)
		assertNull(tube.findIntersections(new Ray(p100, v001)), "Ray is orthogonal and inside center of tube");

		// TC48: Ray is orthogonal and inside center of tube, inside the tube center(0 points)
 ...
 * 	    Vector vOrth;
	    ...
	    if(Util.isZero(vDotVa) || v.equals(va.scale(vDotVa))) return null;
	    else vOrth = v.subtract(va.scale(vDotVa));

	    Vector dpOrth;
	    double dpDotVa = deltaP.dotProduct(va);
	    if(Util.isZero(dpDotVa) || deltaP.equals(va.scale(dpDotVa))) return null;
	    else dpOrth = deltaP.subtract(va.scale(dpDotVa));


	    if (v.equals(va.scale(vDotVa))) return null;
	    
	    if ((!Util.isZero(vDotVa)) && (deltaP.equals(va.scale(dpDotVa)))) c = - radius * radius;
