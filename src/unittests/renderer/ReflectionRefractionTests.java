...
class ReflectionRefractionTests {
	...
	/**
     * Test for stage7 bonus1 scene, "Bonus1 Scene".
     * This method constructs a complex scene designed to fulfill all bonus requirements by
     * showcasing a variety of geometries, materials, and lighting effects. The scene
     * includes over 10 objects of all implemented types (Plane, Sphere, Triangle,
     * Polygon, Cylinder, Tube) and demonstrates advanced rendering features such as
     * reflection, refraction, and complex shadows from multiple light sources.
     */
	@Test
	public void stage7Bonus1() {
        // Use a local Scene and Camera.Builder to ensure test isolation and prevent conflicts.
		Scene myScene = new Scene("Bonus1 Scene")
				.setAmbientLight(new AmbientLight(new Color(20, 20, 30))) // Set a minimal cool ambient light to maintain deep shadows
				.setBackground(new Color(5, 5, 10)); // Set a near-black background

		Camera.Builder myBonusCamera = Camera.getBuilder()
				.setLocation(new Point(-300, -100, 400)) // Position the camera for a wide, cinematic three-quarter view
				.setDirection(new Point(0, -80, 0), new Vector(0, 1, 0)) // Aim towards the center of the composition
				.setViewPlaneDistance(450)
		...
		myScene.geometries.add(
				new Plane(new Point(0, -200, 0), new Vector(0, 1, 0)) // Reflective floor
		...
		// --- Sculpture 1: A large glass pyramid composed of 4 Triangles to demonstrate refraction. ---
		...
		myScene.geometries.add(
				new Triangle(pyrBase1, pyrBase2, pyrTop).setMaterial(glass),
				new Triangle(pyrBase2, pyrBase3, pyrTop).setMaterial(glass),
				new Triangle(pyrBase3, pyrBase4, pyrTop).setMaterial(glass),
				new Triangle(pyrBase4, pyrBase1, pyrTop).setMaterial(glass)
		);
		...
		myScene.geometries.add(new Sphere(new Point(0, -70, 0), 40).setMaterial(mirror).setEmission(new Color(200,200,200)));
		...
		myScene.geometries.add(new Polygon(p1, p2, p3, p4, p5).setMaterial(metallicGold).setEmission(new Color(180, 150, 90)));
		...
		myScene.geometries.add(new Tube(new Ray(new Point(-400, 100, -100), new Vector(1, -0.2, 0.5)), 5)
				.setEmission(new Color(255, 0, 150)) 
				.setMaterial(new Material())); // Material with no shading properties, only emission.
		...
		myScene.geometries.add(new Sphere(new Point(200, -170, 50), 30).setMaterial(matteRed).setEmission(new Color(RED)));
		
		// 4. A small green Sphere with a glossy finish.
		myScene.geometries.add(new Sphere(new Point(150, -100, -50), 35).setMaterial(glossyGreen).setEmission(new Color(GREEN)));


		// --- Lighting Configuration (3-Point Lighting) ---
		// Key Light: A sharp spotlight to cast dramatic, hard-edged shadows and create strong highlights.
		myScene.lights.add(new SpotLight(new Color(700, 500, 400), new Point(-300, 100, 200), new Vector(1, -0.5, -0.75))
				.setKl(0.00001).setKq(0.000005));
		
		// Fill Light: A distant point light to introduce a cool color tint to the shadowed areas.
		myScene.lights.add(new PointLight(new Color(100, 150, 255), new Point(300, 50, 300))
				.setKl(0.0001).setKq(0.00001));

		// Rim Light: A directional light to create subtle highlights on object edges, separating them from the background.
    ...
    /**
     * A utility method to construct a box (cuboid) from 6 Polygons and add it to a given Scene.
     ...
     */
	private void addBoxToScene(Scene scene, Point center, double width, double height, double depth, Material material, Color emission) {
		double w = width / 2;
		double h = height / 2;
		double d = depth / 2;
